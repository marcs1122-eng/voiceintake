...
export default function AdminDashboard() {
  ...
  async function downloadPDF(intake) {
    try {
      const res = await fetch(`/api/admin/pdf/${intake.callSid}`, {
        headers: { Authorization: `Bearer ${password}` },
      });
      ...
      a.href = url;
      a.download = `intake-${(intake.patientName || 'unknown').replace(/\s+/g, '-').toLowerCase()}.pdf`;
