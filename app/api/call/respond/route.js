...
const NEW_PATIENT_QUESTIONS = [
  ...
  { field: 'treatments', question: 'What treatments have you tried so far?' },
  ...
  { field: 'conditions', question: 'Any medical conditions like diabetes, high blood pressure, or heart disease?' },
  ...
  { field: 'ros_msk', question: 'Any other joint pain or limited range of motion beyond what you mentioned?' },
...
const FOLLOWUP_QUESTIONS = [
  ...
  { field: 'pain_desc', question: 'How would you describe the pain?' },
  ...
  { field: 'soc_chg', question: 'Any changes in your personal situation?' },
...
const PROGRESS_MESSAGES = {
  8: "You're doing great — about halfway there.",
  16: "Almost done, just a few more.",
  28: "Nearly finished now.",
};
...
export async function POST(request) {
  ...
  if (session.questionIndex >= questions.length) {
    ...
    const finalText = `${reply} Your intake is complete. The clinical team will have everything ready before your appointment. Thanks so much and have a great day!`;
    return respondWithAudio(finalText, flowType, true);
  }

  // Save session non-blocking — don't await
  kv.set(`session:${callSid}`, session, { ex: 3600 }).catch(() => {});
...
async function processWithClaude(speech, session, questions) {
  ...
  const systemPrompt = `You are Sarah, a warm intake assistant for Global Neuro and Spine Institute. You're on a phone call. Be natural and human — like a caring nurse, not a robot or a form.

CRITICAL — replies are spoken aloud on a phone:
- Max 1-2 short sentences. Never longer.
- When advancing: briefly acknowledge + ask next question in ONE reply.
- Never repeat a question already answered.
- Use contractions. Never say "Great!", "Certainly!", "Absolutely!".
- ${firstName ? `Use ${firstName}'s name occasionally.` : 'Keep a warm tone.'}
- Off-topic question: answer briefly in one sentence, return to intake.

MEDICAL: "water pill"=diuretic, "sugar"=diabetes, "blood thinner"=anticoagulant, "bad back"=lumbar pain, "burning down leg"=radicular pain. Accept: "yeah", "nope", "nah", "kind of".

SPECIAL FIELDS:
- name_spelling: Reply exactly: "Got it — [First] spelled [F-I-R-S-T], [Last] spelled [L-A-S-T]. Is that right?"
- name_confirm: Yes=advance. No=fix spelling, stay, re-confirm.

CURRENT:
- Visit: ${session.flowType === 'followup' ? 'Follow-up' : 'New patient'}
- Field: ${currentQ?.field} — "${currentQ?.question}"
- Next: "${nextQ?.question || 'Done — wrap up warmly.'}"
- Collected: ${Object.keys(session.allResponses).join(', ') || 'nothing yet'}
- Name: ${session.allResponses['full_name'] || 'unknown'}
- Spelling: ${session.allResponses['name_spelling'] || 'not yet'}
...
<Response>
  <Play>${audioUrl}</Play>
    ...
    twiml = `<?xml version="1.0" encoding="UTF-8"?>
<Response>
  <Gather input="speech" action="${actionUrl}" method="POST" speechTimeout="1" speechModel="phone_call" enhanced="true" language="en-US" timeout="10">
    <Play>${audioUrl}</Play>
  </Gather>
  <Gather input="speech" action="${actionUrl}" method="POST" speechTimeout="1" speechModel="phone_call" enhanced="true" language="en-US" timeout="10">
...
const PROGRESS_MESSAGES = {
  8: "You're doing great, about halfway through.",
  16: "Almost there, just a few more.",
  28: "Nearly done now.",
};

// Fire-and-forget PDF generation and email delivery
async function triggerPDF(session, callDuration) {
  try {
    const transcript = session.chatHistory.map(h => ({
      role: h.role === 'assistant' ? 'agent' : 'user',
    ...
    await fetch(`${BASE_URL}/api/call/complete`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        data: {
    ...
    console.error(`[respond] PDF trigger failed:`, err.message);
  }
}
...
export async function POST(request) {
  ...
  if (session.questionIndex >= questions.length) {
    ...
    // Trigger PDF and email (fire and forget — don't block the call)
    triggerPDF(session, callDuration);
