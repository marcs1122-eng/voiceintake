...
function triggerPDF(session, callDuration) {
  ...
  fetch(`${BASE_URL}/api/call/complete`, {
    ...
    .then(res => {
      if (!res.ok) console.error(`[respond] PDF trigger HTTP ${res.status} for ${session.callSid}`);
      else console.log(`[respond] PDF triggered for ${session.callSid}`);
    })
