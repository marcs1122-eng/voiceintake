...
const CORR = [
    "go back","wait","that's wrong","thats wrong","that is wrong","no no",
    "let me fix","let me correct","i meant","i said","my name is spelled",
    "spelled","not right","incorrect","redo","repeat","can you ask","ask again",
    "wrong","back up","hold on","sorry","correction","actually"
  ];
function isCor(t) {
    const l = t.toLowerCase().trim();
    return CORR.some((p) => l.includes(p));
}
...
async function speakEL(t) {
    try {
          ...
          return new Promise((v) => {
                  const a = new Audio(u);
                  a.onended = () => { URL.revokeObjectURL(u); v(); };
                  a.onerror = () => { URL.revokeObjectURL(u); v(); };
                  a.play().catch(() => v());
          });
    } catch (e) {
          return new Promise((v) => {
...
export default function VoiceIntake() {
  ...
  const getActive = useCallback(
        (i, d) => {
                while (i < flow.length) {
                          if (flow[i].c && !flow[i].c(d)) { i++; continue; }
                          return i;
                }
                return flow.length;
        },
  ...
  const doSubmit = useCallback(
        async (v) => {
                ...
                const cr = respRef.current;
                const idx = getActive(cs, cr);
                if (idx >= flow.length) return;

          if (isCor(v)) {
                    let pi = idx - 1;
                    while (pi >= 0 && flow[pi].c && !flow[pi].c(cr)) pi--;
                    if (pi >= 0) {
                                const ur = { ...cr };
                                delete ur[flow[pi].f];
                                setResp(ur);
                                ...
                                const gm = "No problem, let me ask that again. " + flow[pi].q;
                                setChat((p) => [...p, { role: "ai", text: gm, id: Date.now() + Math.random() }]);
                                if (voiceOnRef.current) {
                                              ...
                                              setTimeout(() => {
                                                              if (voiceOnRef.current && !busyRef.current && startLRef.current) startLRef.current();
                                              }, 300);
                                }
                                return;
                    }
          ...
          const nr = { ...cr, [flow[idx].f]: v };
                setResp(nr);
                respRef.current = nr;
                setChat((p) => [...p, { role: "user", text: v, id: Date.now() + Math.random() }]);
                const nx = getActive(idx + 1, nr);
                if (nx >= flow.length) {
                          const cl = "That is everything I need! Let me put your summary together. Thank you for your patience!";
  ...
  const startL = useCallback(() => {
        ...
        r.onresult = (e) => {
                ...
                for (let i = e.resultIndex; i < e.results.length; i++) {
                          ...
                          else n += e.results[i][0].transcript;
                }
                if (f) {
                          ...
                          timerRef.current = setTimeout(() => {
                                      if (pendingRef.current.trim() && !busyRef.current) {
                                                    const s = pendingRef.current.trim();
                                                    pendingRef.current = "";
  ...
  const beginIntake = async (t) => {
        ...
        setIsSpeaking(false);
        busyRef.current = false;
        setTimeout(() => { if (startLRef.current) startLRef.current(); }, 300);
  };
  ...
  const c = {
        bg: "#070B14", card: "#0F1623", border: "#1A2438", pri: "#3B82F6",
        priG: "rgba(59,130,246,.12)", sec: "#10B981", txt: "#E8EDF5",
        mut: "#8896AB", dim: "#5A6A80", red: "#EF4444", acc: "#F59E0B",
        surf: "#162032", aiBg: "#0D1825", userBg: "#142847",
  };
  ...
  if (screen === "home")
        ...
        <div style={{ textAlign: "center", maxWidth: 520 }}>
          ...
          <div style={{ marginTop: 40, padding: "10px 18px", background: c.card, borderRadius: 10, border: "1px solid " + c.border, display: "inline-block", fontSize: 12 }}>
            ...
            <span style={{ fontWeight: 600, marginLeft: 6 }}>Global Neuro & Spine Institute</span>
  </div>
  </div>
  </div>
    );
  ...
  if (screen === "intake")
        ...
        <div style={{ padding: "10px 16px", background: c.card, borderBottom: "1px solid " + c.border, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <button onClick={() => { window.speechSynthesis?.cancel(); stopL(); setScreen("home"); }} style={{ background: "none", border: "none", color: c.mut, cursor: "pointer", fontSize: 20 }}>&#8592;</button>
            <div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <button onClick={() => { const nv = !voiceOn; setVoiceOn(nv); voiceOnRef.current = nv; if (!nv) { window.speechSynthesis?.cancel(); stopL(); } }} style={{ padding: "4px 10px", fontSize: 11, borderRadius: 6, border: "1px solid " + (voiceOn ? c.pri : c.border), background: voiceOn ? c.priG : "transparent", color: voiceOn ? c.pri : c.dim, cursor: "pointer" }}>{voiceOn ? "Sound On" : "Sound Off"}</button>
            <span style={{ fontSize: 12, color: c.dim }}>{Math.min(ai + 1, flow.length)}/{flow.length}</span>
    </div>
    </div>
        <div style={{ height: 3, background: c.surf }}>
          <div style={{ height: "100%", width: prog + "%", background: "linear-gradient(90deg,#3B82F6,#10B981)", transition: "width .5s" }} />
    </div>
...
{isSpeaking && <div style={{ padding: "6px 16px", background: "rgba(59,130,246,.04)" }}><span style={{ fontSize: 11, color: c.pri }}>Speaking...</span></div>}
{isListening && !isSpeaking && <div style={{ padding: "6px 16px", background: "rgba(239,68,68,.04)" }}><span style={{ fontSize: 11, color: c.red }}>Listening... speak your answer</span></div>}
...
{Array.from({ length: 11 }, (_, n) => (
                    <button key={n} onClick={() => doSubmit(String(n))} style={{ width: 38, height: 38, borderRadius: 8, border: "1px solid " + c.border, background: n <= 3 ? "rgba(16,185,129,.08)" : n <= 6 ? "rgba(245,158,11,.08)" : "rgba(239,68,68,.08)", color: n <= 3 ? c.sec : n <= 6 ? c.acc : c.red, fontSize: 14, fontWeight: 600, cursor: "pointer" }}>{n}</button>
                ))}
                  </div>
            )}
</div>
        ...
        <div style={{ padding: "10px 16px 16px", background: c.card, borderTop: "1px solid " + c.border }}>
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <button onClick={() => { if (isListening) stopL(); else startL(); }} disabled={isSpeaking} style={{ width: 48, height: 48, borderRadius: "50%", border: "none", background: isListening ? "linear-gradient(135deg,#EF4444,#DC2626)" : "linear-gradient(135deg,#3B82F6,#2563EB)", color: "#fff", cursor: isSpeaking ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", opacity: isSpeaking ? 0.4 : 1 }}>
...
{resp.full_name && (
                <div style={{ marginTop: 14, padding: "10px 14px", background: c.priG, borderRadius: 8 }}>
...
{Object.entries(g).map(([s, items]) => (
              ...
              <div style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: ".08em", color: c.pri, marginBottom: 10, paddingBottom: 6, borderBottom: "1px solid " + c.border }}>{s}</div>
                       {items.map((i) => (
                  <div key={i.f} style={{ padding: "8px 12px", background: c.card, borderRadius: 8, border: "1px solid " + c.border, marginBottom: 8 }}>
                  <div style={{ fontSize: 10, color: c.dim, textTransform: "capitalize" }}>{i.f.replace(/_/g, " ")}</div>
                  <div style={{ fontSize: 14 }}>{i.a}</div>
  </div>
              ))}
                </div>
          ))}
          <div style={{ marginTop: 28, padding: "18px 20px", background: "rgba(16,185,129,.05)", borderRadius: 12, border: "1px solid rgba(16,185,129,.15)", textAlign: "center" }}>
            <div style={{ fontSize: 15, fontWeight: 600, color: c.sec }}>Intake Complete</div>
            <div style={{ fontSize: 12, color: c.mut, marginTop: 3 }}>Ready to deliver as PDF via secure email.</div>
            </div>
            </div>
            </div>
    );
